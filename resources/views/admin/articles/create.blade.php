...
@section('page_title', 'Professional Newsroom - Write News Story')
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div class="d-flex align-items-center gap-3">
        ...
        <div>
            <h1 class="h3 mb-0 fw-bold text-gray-800"><i class="fas fa-newspaper text-primary me-2"></i>Professional News Editor</h1>
            <p class="text-muted small mb-0">Write, format, and publish high-quality news articles for your portal.</p>
        </div>
    </div>
</div>

<form action="{{ route('admin.articles.store') }}" method="POST" enctype="multipart/form-data" id="newsArticleForm">
    @csrf
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-8">
            ...
            <div class="card border-0 shadow-sm rounded-4 p-4 mb-4">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="title" id="title" class="form-control form-control-lg fw-bold rounded-3 @error('title') is-invalid @enderror" placeholder="Enter bold, captivating headline title..." value="{{ old('title') }}" required style="font-size: 1.4rem;">
                    @error('title')
                ...
                <div class="mb-4">
                    <label for="slug" class="form-label small fw-bold text-uppercase text-muted">URL Slug (Auto Generated)</label>
                    <div class="input-group">
                        <span class="input-group-text bg-light border-end-0 small text-muted">/news/</span>
                        <input type="text" name="slug" id="slug" class="form-control rounded-end-3 @error('slug') is-invalid @enderror" placeholder="news-url-slug" value="{{ old('slug') }}">
                    </div>
                    <div class="form-text small text-muted">Auto-generated from title. You can customize it if needed.</div>
                    @error('slug')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="subtitle" id="subtitle" class="form-control rounded-3 @error('subtitle') is-invalid @enderror" placeholder="Enter secondary headline summary that hooks the reader..." value="{{ old('subtitle') }}">
                    @error('subtitle')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="summary" id="summary" rows="3" class="form-control rounded-3 @error('summary') is-invalid @enderror" placeholder="Brief 2-3 sentence overview for search engines and social cards...">{{ old('summary') }}</textarea>
                    @error('summary')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="p-4 rounded-4 bg-body-tertiary border mb-4">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="media_type" id="media_type" class="form-select rounded-3 @error('media_type') is-invalid @enderror" required>
                            <option value="image_file" {{ old('media_type', 'image_file') == 'image_file' ? 'selected' : '' }}>Upload Image File</option>
                            <option value="image_link" {{ old('media_type') == 'image_link' ? 'selected' : '' }}>External Image URL Link</option>
                            <option value="video_file" {{ old('media_type') == 'video_file' ? 'selected' : '' }}>Upload Video File</option>
                    ...
                    <div id="media-field-image-file" class="media-input-group mb-0">
                        <label for="image" class="form-label small fw-semibold">Select Landscape Featured Image</label>
                        <input type="file" name="image" id="image" class="form-control rounded-3 @error('image') is-invalid @enderror" accept="image/*">
                        <div class="form-text small text-muted">High-resolution landscape photo (JPEG, PNG, WEBP).</div>
                        @error('image')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div id="media-field-video-file" class="media-input-group mb-0" style="display: none;">
                        ...
                        <div class="form-text small text-muted">Video file (MP4, WEBM). Max 20MB.</div>
                        @error('video')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div id="media-field-link" class="media-input-group mb-0" style="display: none;">
                        ...
                        <input type="url" name="media_link" id="media_link" class="form-control rounded-3 @error('media_link') is-invalid @enderror" placeholder="https://..." value="{{ old('media_link') }}">
                        <div class="form-text small text-muted" id="media-link-help">Enter full URL link to image or YouTube video.</div>
                        @error('media_link')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
        ...
        <div class="col-lg-4">
            <div class="sticky-top" style="top: 20px; z-index: 10;">
                ...
                <div class="card border-0 shadow-sm rounded-4 p-4 mb-4">
                    <h6 class="fw-bold mb-3 text-primary border-bottom pb-2"><i class="fas fa-paper-plane me-2"></i>Publishing & Status</h6>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="form-text small text-muted">Leave empty to publish live immediately.</div>
                    </div>

                    <!-- Tags Input -->
                    <div class="mb-3">
                        <label for="tags" class="form-label small fw-bold text-muted text-uppercase">Tags (Comma Separated)</label>
                        <input type="text" name="tags" id="tags" class="form-control rounded-3" placeholder="e.g. Budget2026, Uttarakhand, Politics" value="{{ old('tags') }}">
                        <div class="form-text small text-muted">Adds clickable hashtag badges.</div>
                    </div>
                    ...
                    <div class="mb-4">
                        <h6 class="small fw-bold text-uppercase text-muted mb-2"><i class="fas fa-chart-bar me-1"></i>Live Article Metrics</h6>
                    ...
                    <div class="d-grid gap-2">
                        <button type="submit" class="btn btn-primary rounded-pill py-2.5 fw-bold shadow-sm">
                            <i class="fas fa-globe me-2"></i>Publish Story Now
                        </button>
                ...
                <div class="news-preview-card p-3 shadow-sm mb-4">
                    ...
                    <div class="position-relative mb-2 rounded-3 overflow-hidden" style="aspect-ratio: 16/9; background: #222;">
                        <img id="previewImage" src="https://images.unsplash.com/photo-1585829365295-ab7cd400c167?auto=format&fit=crop&w=600&q=80" class="w-100 h-100 object-fit-cover" alt="Preview">
                    </div>
                    <h6 class="fw-bold fs-7 mb-1 line-clamp-2" id="previewTitle">Article headline preview will appear here...</h6>
                    <small class="text-muted fs-9" id="previewMeta">Rivaaz News • Just Now</small>
                </div>
            </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#summernote').summernote({
            placeholder: 'Write your news article here in rich detail... You can paste formatted text, insert images, format headings, blockquotes, and add embedded media.',
            tabsize: 2,
            height: 420,
            toolbar: [
                ['style', ['style']],
        ...
        titleInput.addEventListener('input', function() {
            ...
            previewTitle.textContent = val.trim() ? val : 'Article headline preview will appear here...';
        ...
        function updateMetricsFromText() {
            const htmlContent = $('#summernote').summernote('code');
            updateArticleMetrics(htmlContent);
        }
    });
</script>
@endsection
