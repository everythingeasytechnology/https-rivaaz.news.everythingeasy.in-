    ...
    <div class="col-lg-8">
        <div class="admin-card">
            <div class="d-flex align-items-center gap-2 mb-4">
                ...
                <h5 class="fw-bold mb-0">Modify Category: {{ $category->name }}</h5>
            </div>
            ...
            <form action="{{ route('admin.categories.update', $category->id) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="name" id="name" class="form-control rounded-3 @error('name') is-invalid @enderror" placeholder="e.g. Science" value="{{ old('name', $category->name) }}" required>
                    @error('name')
                ...
                <!-- Slug -->
                <div class="mb-3">
                    <label for="slug" class="form-label small fw-bold text-uppercase">Custom URL Slug</label>
                    <input type="text" name="slug" id="slug" class="form-control rounded-3 @error('slug') is-invalid @enderror" placeholder="e.g. science-and-tech" value="{{ old('slug', $category->slug) }}">
                    <div class="form-text small text-muted">Unique URL segment identifying this category. Changing this will alter page links.</div>
                    @error('slug')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <div class="form-text small text-muted">Select a parent category if you want to make this a subcategory.</div>
                    @error('parent_id')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="p-4 rounded-4 bg-body-tertiary border mb-4">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="meta_title" id="meta_title" class="form-control rounded-3" placeholder="e.g. Science & Space Technology News - Rivaaz" value="{{ old('meta_title', $category->meta_title) }}">
                    </div>

                    <!-- Meta Description -->
                    <div class="mb-3">
                        <label for="meta_description" class="form-label small fw-semibold">Meta Description</label>
                        <textarea name="meta_description" id="meta_description" rows="3" class="form-control rounded-3" placeholder="e.g. Read the latest coverage and scientific breakthroughs...">{{ old('meta_description', $category->meta_description) }}</textarea>
                    </div>

                    <!-- Meta Keywords -->
                    <div class="mb-0">
                        <label for="meta_keywords" class="form-label small fw-semibold">Meta Keywords</label>
                        <input type="text" name="meta_keywords" id="meta_keywords" class="form-control rounded-3" placeholder="e.g. science, space, physics, tech" value="{{ old('meta_keywords', $category->meta_keywords) }}">
                    </div>
                </div>
        ...
        <div class="admin-card">
            ...
            <div class="border rounded-3 p-3 bg-white text-dark shadow-sm" style="font-family: Arial, sans-serif;">
                <div class="text-truncate text-primary fs-6 mb-1" id="seo-preview-title" style="color: #1a0dab !important; font-weight: normal; cursor: pointer; text-decoration: none;">
                    {{ $category->meta_title ?: $category->name . ' News - Rivaaz Chronicle' }}
                </div>
                <div class="text-truncate text-success small mb-1" style="color: #006621 !important;">
                    http://127.0.0.1:8000/category/<span id="seo-preview-slug">{{ $category->slug }}</span>
                </div>
                <div class="small text-muted" id="seo-preview-desc" style="color: #545454 !important; line-height: 1.4;">
                    {{ $category->meta_description ?: 'Latest updates and news from the ' . $category->name . ' category page.' }}
                </div>
            </div>
        </div>
    </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        nameInput.addEventListener('input', function() {
            if (!slugInput.value || slugInput.dataset.edited === 'false') {
                ...
                previewSlug.textContent = slug || 'category';
            }
            updatePreview();
        });
        ...
        slugInput.addEventListener('input', function() {
            ...
            previewSlug.textContent = generateSlug(slugInput.value) || 'category';
        });

        metaTitleInput.addEventListener('input', updatePreview);
        metaDescInput.addEventListener('input', updatePreview);

        slugInput.dataset.edited = 'true'; // Set true since it starts populated

        function updatePreview() {
            previewTitle.textContent = metaTitleInput.value || (nameInput.value ? nameInput.value + ' News - Rivaaz Chronicle' : 'Category News - Rivaaz Chronicle');
            previewDesc.textContent = metaDescInput.value || 'Latest updates and news from the category page.';
        }
    });
</script>
@endsection
